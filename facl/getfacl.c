...
struct name_list *get_list(struct stat *st, acl_t acl)
{
...
int do_show(FILE *stream, const char *path_p, struct stat *st,
...
int do_print(const char *path_p, struct stat *st, void *dummy)
{
	const char *str;
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, cmd_line_options,
		...
		switch (opt) {
			...
			case 'H':  /* follow symlink arguments */
				if (posixly_correct)
					goto synopsis;
	...
	do {
		if (optind == argc ||
			...
			while ((line = next_line(stdin)) != NULL) {
				...
				had_errors += walk_tree(line, do_print, NULL);
			}
			if (ferror(stdin)) {
			...
			had_errors += walk_tree(argv[optind], do_print, NULL);
		optind++;
	} while (optind < argc);
