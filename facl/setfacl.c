...
int opt_recalculate = 0;  /* recalculate mask entry (0=default, 1=yes, -1=no) */
int opt_promote = 0;  /* promote access ACL to default ACL */
int opt_test = 0;  /* do not write to the file system.
	...
	const char *filename)
{
	...
	for(;;) {
		...
		error = do_set(path_p, &stat, (void *)seq);
		if (error != 0) {
			status = 1;
			goto resume;
		}
...
int next_file(const char *arg, seq_t seq)
{
	...
	if (strcmp(arg, "-") == 0) {
		while ((line = next_line(stdin)))
			errors += walk_tree(line, do_set, (void*)seq);
	} else {
		errors += walk_tree(arg, do_set, (void *)seq);
	}
	return errors ? 1 : 0;
}
